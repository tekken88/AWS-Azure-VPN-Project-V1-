...
resource "azurerm_local_network_gateway" "localgwhub" {
  ...
  location            = azurerm_virtual_network.frontend_vnet_east_1.location
...
resource "azurerm_local_network_gateway" "localgwonsite" {
  name                = "ONSITE"
  resource_group_name = var.resource_group_name
  location            = azurerm_virtual_network.vnet_west_onsite_1.location
  gateway_address     = azurerm_public_ip.westus2-vpn-gateway-pip.ip_address
  address_space       = azurerm_virtual_network.vnet_west_onsite_1.address_space
